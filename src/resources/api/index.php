...
function deleteResource($db, $resourceId) {
    ...
    try {
        ...
        $c = $db->prepare("DELETE FROM comments WHERE resource_id=?");
        $c->execute([$resourceId]);

        $r = $db->prepare("DELETE FROM resources WHERE id=?");
        $r->execute([$resourceId]);
